...
export const getScreams = () => (dispatch) => {
 // console.log("finding out what dispatch is", dispatch);
  ...
  axios
    ...
    .then((res) => {
      ...
      if (res.data.length !== 0) {
        ...
        //console.log("Else statement of getScreams runs");
        const data = {
          userHandle: "user",
          body:
            "this is a dummy scream used for debug purposes, code should be removed in production",
       ...
       // console.log("%c [dummy code runs]", "color:pink", data);
        dispatch({
          type: SET_SCREAM,
          payload: data,
        });
        dispatch({ type: STOP_LOADING_UI });
        console.log("%c [Data from getScream]", "color:pink", data);
      }
    })
    .catch((err) =>
...
export const likeScream = (screamId) => (dispatch) => {
  //console.log("likeScream in dataActions runs");
  axios
    .get(`/scream/${screamId}/like`)
    .then((res) => {
     /*  console.log(
        "axios get in likescream in dataActions runs, this is res",
        res
      ); */
